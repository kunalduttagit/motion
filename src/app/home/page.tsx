...
import { Card, Card2, Card3, CardList } from '@/components/ui/card';
...
export default function CarouselPlugin() {
    ...
    return (
        <main className='w-screen text-white footer-wrap'>
           ...
           {renderMovies() && (<div className={`text-white text-9xl absolute  ${scrollPercentage > 100 ? "bg-black " : "bg-gradient-to-b from-transparent from-5% via-black via-20% to-black to-95%  "} top-[70vh] h-[100%] bottom-0 left-0 right-0`} >
                ...
                <div className='ml-6'>
                    ...
                    <ul className='mt-2 flex overflow-x-scroll pb-10 hide-scroll-bar no-scrollbar'>
                        {movies.popular?.map((movie:any) => (
                                <li key={movie._id}>
                                    <Card3 id={movie._id} imageUrl={movie.image} title={movie.title} genres={movie.genre} year={movie.year} watchListProp={watchlist?.includes(movie._id)}/>
                                </li>
                            ))
                        }
                    </ul>
                ...
                {recommendations.length > 0 && (<div>
	                 <div className='text-3xl ml-6 font-semibold flex w-[100vw] justify-start'>
	                     {recommendations.length > 1 && <><span className=''>Top pics for</span> <GradientName username={username}/> </>}
	                 </div>
	                 <div className=' background-blur-list p-6'>
	                     <ul className='mt-2 flex overflow-x-scroll hide-scroll-bar no-scrollbar'>
	                         {recommendations?.map((movie:any) => (
	                                 <li key={movie._id}>
	                                     <CardList id={movie._id} imageUrl={movie.pimage} title={movie.title} genres={movie.genre} year={movie.year} watchListProp={watchlist?.includes(movie._id)}/>
	                                 </li>
	                             ))
	                         }
	                     </ul>
                        ...
                        <div key={index} className='ml-6'>
                            ...
                            <div>
                                ...
                                {movies[genre.replaceAll(' ', '').toLowerCase()]?.map((movie: Movie) => (
                                    <li key={movie._id}>
                                        <Card3 id={movie._id} imageUrl={movie.image} genres='' title={movie.title} year={movie.year} watchListProp={watchlist?.includes(movie._id)} />
                                    </li>
